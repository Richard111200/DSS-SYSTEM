...
let lastChosenAnime = localStorage.getItem("lastChosenAnime")
    ? JSON.parse(localStorage.getItem("lastChosenAnime"))
    : null;
...
function getBotResponse(userMessage) {
    ...
    if (lowerCaseMessage.includes('action')) {
        ...
        localStorage.removeItem("lastChosenAnime");
    ...
    if (chosenIndex >= 0 && chosenIndex < actionAnimeRecommendations.length) {
        ...
        localStorage.setItem("lastChosenAnime", JSON.stringify(lastChosenAnime));
...
// On page load, display the last chosen anime if it exists
if (lastChosenAnime) {
    addMessage(`Welcome back! You were last asking about "${lastChosenAnime.title}".`, false);
}
